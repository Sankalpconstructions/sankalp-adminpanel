...
const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      ...
      {/* Multiple Preview */}
{multiple && Array.isArray(cleanedValue) && cleanedValue.length > 0 && (
  <div className="flex flex-wrap gap-3 mt-4">
    {cleanedValue.map((url, idx) => (
...
{/* Single Preview */}
{!multiple && typeof cleanedValue === "string" && cleanedValue && (
  <div className="relative w-full h-32 rounded-xl overflow-hidden mt-2 border border-gray-100 shadow-sm group">
    <img src={cleanedValue} alt="preview" className="w-full h-full object-cover" />
    <button
      type="button"
      onClick={() => removeImage(cleanedValue)}
