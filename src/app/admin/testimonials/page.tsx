...
import { Plus, Edit2, Trash2, Search, X, Check, Star, Quote } from "lucide-react";
import { initialTestimonials } from "@/lib/mockData";
...
export default function TestimonialsAdminPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingItem) {
      ...
      setTestimonials([{ ...formData, id: Math.max(...testimonials.map(t => t.id), 0) + 1 }, ...testimonials]);
    }
          ...
          {filtered.map(item => (
            ...
            >
              <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                ...
                <button onClick={() => handleDelete(item.id)} className="p-2 bg-red-50 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition-all"><Trash2 size={14} /></button>
              </div>
