...
import { Plus, Edit2, Trash2, Search, Check, ArrowLeft, RefreshCw, Store } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import ImageUpload from "@/components/admin/ImageUpload";
import toast from "react-hot-toast";

export default function ResaleCommercialAdminPage() {
  ...
  const fetchProperties = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/resale?type=commercial");
      if (res.ok) {
        ...
        if (Array.isArray(data)) {
          ...
          setProperties([]);
          console.error("Expected array from resale API, got:", data);
        }
      } else {
        setProperties([]);
        console.error("Failed to fetch resale properties, status:", res.status);
      }
    } catch (error) {
      console.error("Error fetching commercial resale properties:", error);
      setProperties([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleOpenForm = (property: any = null) => {
    if (property) {
      ...
      setFormData({
        title: "", location: "", image: "", sqft: "", facing: "", phone: "", whatsapp: "", status: "available", propertyType: "commercial"
      });
    }
    setIsFormOpen(true);
  };
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure you want to delete this property?")) {
      try {
        const res = await fetch(`/api/resale/${id}`, { method: "DELETE" });
        if (res.ok) {
          setProperties(properties.filter(p => (p._id || p.id) !== id));
        
          toast.success("Commercial property saved successfully!");
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.title || !formData.location || !formData.sqft || !formData.facing || !formData.phone || !formData.whatsapp) {
    ...
    try {
      if (editingProperty) {
        ...
        const res = await fetch(`/api/resale/${id}`, {
          method: "PUT",
        ...
        {!isFormOpen ? (
          <motion.div key="list" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h1 className="text-2xl font-bold text-gray-900 uppercase tracking-tight">Commercial Resales</h1>
                <p className="text-gray-500 text-sm">Manage commercial properties for resale.</p>
              </div>
              <button 
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-sm">Enter the details for the commercial resale property.</p>
                </div>
              </div>
            ...
            <section className="bg-white rounded-2xl border border-gray-200/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-8 max-w-4xl">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="col-span-1 md:col-span-2">
                  ...
                  <input required value={formData.title} onChange={(e) => setFormData({...formData, title: e.target.value})} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:border-[#29B1D2]" placeholder="e.g. Premium Office Space" />
                </div>
                <div>
                  <label className="text-[10px] font-bold uppercase text-gray-400 tracking-widest block mb-2">Location *</label>
                  <input required value={formData.location} onChange={(e) => setFormData({...formData, location: e.target.value})} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:border-[#29B1D2]" placeholder="e.g. HITEC City, Hyderabad" />
                </div>
                <div className="col-span-1 md:col-span-2">
                  <ImageUpload 
                ...
                <div>
                  ...
                  <input required value={formData.sqft} onChange={(e) => setFormData({...formData, sqft: e.target.value})} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:border-[#29B1D2]" placeholder="e.g. 5,000 Sq.ft" />
                </div>
                <div>
                  <label className="text-[10px] font-bold uppercase text-gray-400 tracking-widest block mb-2">Facing *</label>
                  <input required value={formData.facing} onChange={(e) => setFormData({...formData, facing: e.target.value})} className="w-full p-4 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:border-[#29B1D2]" placeholder="e.g. North-East" />
                </div>
                <div>
