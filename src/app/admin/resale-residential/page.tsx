...
export default function ResaleResidentialAdminPage() {
  ...
  const fetchProperties = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/resale?type=residential");
      if (res.ok) {
      ...
      console.error("Error fetching residential resale properties:", error);
      setProperties([]);
    } finally {
      setIsLoading(false);
    }
        ...
        {!isFormOpen ? (
          <motion.div key="list" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h1 className="text-2xl font-bold text-gray-900 uppercase tracking-tight">Residential Resales</h1>
                <p className="text-gray-500 text-sm">Manage residential properties for resale.</p>
              </div>
              <button 
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-500 text-sm">Enter the details for the residential resale property.</p>
                </div>
              </div>
